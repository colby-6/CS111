...
def helper_pingpong(n,iterator,current,direction):
    if iterator== n:
        return current
    else: 
        if iterator % 8 == 0 or num_eights(iterator):
            return helper_pingpong(n, iterator + 1, current - direction, -direction)
        else: 
            return helper_pingpong(n, iterator + 1, current + direction, direction)
