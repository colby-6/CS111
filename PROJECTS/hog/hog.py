...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            new = score0 - current_score
            if new > running_high:
                print(f"{new} point(s)! That's a record gain for Player 0!")
            else: 
                new = running_high
            current_score = score0
        elif who == 1:
            new = score1 - current_score
            if new > running_high:
                print(f"{new} point(s)! That's a record gain for Player 1!")
            else: 
                new = running_high
            current_score = score1
        return announce_highest(who, current_score, new)
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    dice_rolls = 1
    while dice_rolls <= 10:
        avrg_max = make_averaged(roll_dice, trials_count)
        counter_maxi = avrg_max(dice_rolls, dice)
        if counter_max < counter_maxi:
            counter_maxi = counter_max
            best_roll = dice_rolls
        dice_rolls += 1
    return best_roll
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0:
        return 0 
    elif hog_pile(new_score, opponent_score) != 0:
        return 0 
    else: return num_rolls
    # END PROBLEM 11
